...
def write_build_marker() -> None:
    """Record build-time feature flags before kernel config selection.

    default_compiled_kernels.py reads build markers so the generated/test kernel
    list matches the compiled extension even if the environment changes later.
    Write both a temporary build-utils marker (for setup-time config selection)
    and a pearl_gemm package marker that is included in the built wheel.
    """
    marker_text = (
        "# Auto-generated by pearl-gemm setup.py; do not edit.\n"
        f"FUSE_NOISE_B = {bool(FUSE_NOISE_B)}\n"
    )
    marker_paths = [ROOT_DIR / "src" / "pearl_gemm" / "_build_marker.py"]
    try:
        import pearl_gemm_build_utils.kernel_configs as kernel_config_pkg

        marker_paths.append(Path(kernel_config_pkg.__file__).parent / "_build_marker.py")
    except Exception as exc:
        warnings.warn(f"could not locate build-utils marker path: {exc!r}", stacklevel=2)
    for marker_path in marker_paths:
        try:
            marker_path.write_text(marker_text)
            print(f"Wrote build marker {marker_path} (FUSE_NOISE_B={FUSE_NOISE_B})")
        except Exception as exc:
            warnings.warn(f"could not write build marker {marker_path}: {exc!r}", stacklevel=2)
