...
# V3: decoupled BpEB buffer (separate SMEM) so the GEMM consumer reads a clean
# noised-B operand instead of RMWing sB in place. Tests whether decoupling removes
# the per-tile noise barrier stall. Costs an extra B-sized SMEM pipeline.
FUSE_NOISE_B_V3 = _env_flag("PEARL_FUSE_NOISE_B_V3", "FALSE")
FUSE_DEBUG = _env_flag("PEARL_FUSE_DEBUG", "FALSE")
...
FEATURE_FLAGS = {
    ...
    "PEARL_FUSE_NOISE_B_V3": FUSE_NOISE_B_V3,
    "PEARL_FUSE_DEBUG": FUSE_DEBUG,
}
...
if not SKIP_CUDA_BUILD:
    ...
    # Record what was actually built so the runtime/test config matches the
    # compiled kernels regardless of the current env (avoids fused-build vs
    # unset-env stage mismatches). Written next to the kernel config module.
    try:
        import pearl_gemm_build_utils.kernel_configs as _kc_pkg

        _marker_path = Path(_kc_pkg.__file__).parent / "_build_marker.py"
        _fuse_on = FUSE_NOISE_B
        _marker_path.write_text(
            "# Auto-generated by pearl-gemm setup.py; do not edit.\n"
            f"FUSE_NOISE_B = {bool(_fuse_on)}\n"
        )
        print(f"Wrote build marker {_marker_path} (FUSE_NOISE_B={_fuse_on})")
    except Exception as _e:
        warnings.warn(f"could not write build marker: {_e!r}", stacklevel=2)
